...
public class DataLoader {
    ...
    private final static String PROG[] = {   // 아이디, 앨범 아이디, 타이틀, 아티스트
    ...
    private static void load(Context context){
        ...
        if(cursor!=null){
            while(cursor.moveToNext()){
                ...
                // 데이터
                music.setId(getValue(cursor, PROG[0]));
                music.setAlbum_id(getValue(cursor, PROG[1]));
                music.setTitle(getValue(cursor, PROG[2]));
                music.setArtist(getValue(cursor, PROG[3]));
